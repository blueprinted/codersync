...
class BackupController extends AdminbaseController
{
    ...
    public function indexAction()
    {
		...
		foreach($tables as $key => $table) {
			if(C("DB_PREFIX") && strpos($table, C("DB_PREFIX")) !== 0) {
				unset($tables[$key]);
			}
		}
		$this->assign('tables', $tables); //显示所有数据表
        $this->display();
    }
    ...
    public function index_postAction(){
    	if (IS_POST || isset($_GET['dosubmit']))
    	{
    		...
    		} else
    		{
    			...
    			if(!mkdir_recursive(SITE_PATH . $this->backup_path . $this->backup_name, 0777)) {
					$this->error('创建备份目录失败');
				}
    ...
    {
		$this->error('恢复功能已禁用，请联系管理员进行数据恢复操作。');
    ...
    private function _deal_result($tables, $vol, $sizelimit)
    {
        ...
        } else
        {
            ...
            $link= U("Backup/index_post",array("dosubmit"=>1,"type"=>"url","backup_name"=>$this->backup_name,"sizelimit"=>$sizelimit));
